...
class Application {
	...
	static editBook(e) {
		if (e.classList.contains('edit-book')) {
			...
			titleField.value = `${
				e.parentElement.querySelector('.displayed-values-title').textContent
			}`;
			const authorField = form.querySelector('.author');
			authorField.value = `${
				e.parentElement.querySelector('.displayed-values-author').textContent
			}`;
			const pagesField = form.querySelector('.pages');
			pagesField.value = `${
				e.parentElement.querySelector('.displayed-values-pages').textContent
			}`;
			const statusField = form.querySelector('.status');
			statusField.value = `${
				e.parentElement.querySelector('.displayed-values-status').textContent
			}`;
...
document.querySelector('.library-container').addEventListener('click', (e) => {
	...
	window.bookselection = e.target.parentElement.querySelector(
		'.displayed-values-title'
	).textContent;
	SaveData.removeBook(
		e.target.parentElement.querySelector('.displayed-values-title').textContent
	);
});
